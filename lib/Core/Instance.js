...
class Chatify extends EventEmitter {
    ...
    async start() {
        ...
        this.on('message', async (m, sock) => {
            // Pastikan kita bisa mengambil teks dari objek pesan yang sudah diserialisasi
            const body = m.body || m.text || ''; 
            
            // Deteksi prefix (mendukung awalan: \ / ! # . dan lain-lain jika ditambahkan)
            const prefixMatch = /^[\\/!#.]/.exec(body);
            const prefix = prefixMatch ? prefixMatch[0] : null;
            
            // Abaikan jika tidak memakai prefix (berarti obrolan biasa)
            if (!prefix) return; 
